...
const TOWER_TYPES = {
  basic: {
    ...
    shape: "star",
    name: "Basic Tower",
    description: "Balanced tower for beginners. Good cost-benefit ratio.",
  },
  fast: {
    cost: 75,
    damage: 15,
    ...
    shape: "square",
    name: "Fast Tower",
    description: "Shoot rapidly, ideal for enemies in groups.",
  },
  strong: {
    cost: 100,
    damage: 40,
    ...
    shape: "pentagon",
    name: "Strong Tower",
    description: "High Damage, ideal for resistive enemies.",
  },
  sniper: {
    cost: 150,
    damage: 60,
    ...
    shape: "diamond",
    name: "Sniper Tower",
    description: "Long range and high damage, but shoots slower.",
  },
  splash: {
    cost: 120,
    damage: 25,
    ...
    description: "Causes damage in an area, attacks multiple enemies.",
  },
  slow: {
    cost: 90,
    damage: 8,
    ...
    shape: "square",
    name: "Ice Tower",
    description: "Slows down enemies, allowing strategic control.",
    slowFactor: 0.5,
    slowDuration: 3000,
  },
  poison: {
    ...
    shape: "circle",
    name: "Poison Tower",
    description: "Poison enemies causing continuous damage.",
    poisonDamage: 3,
    poisonDuration: 4000,
  },
};
...
function isValidTowerPosition(x, y) {
  ...
  for (let i = 0; i < PATH.length - 1; i++) {
    ...
    const end = PATH[i + 1];
    if (
      isPointOnPathSegment(x + GRID_SIZE / 2, y + GRID_SIZE / 2, start, end, 30)
    ) {
      return false;
    }
  }
    ...
    if (enemy.pathIndex < PATH.length - 1) {
      ...
      const effectiveSpeed =
        enemy.slowedUntil && Date.now() < enemy.slowedUntil
          ? enemy.speed * (enemy.slowFactor || 1)
          : enemy.speed;

      // Process poison damage
      if (enemy.poisonedUntil && Date.now() < enemy.poisonedUntil) {
        if (Date.now() - enemy.lastPoisonDamage >= 1000) {
          // Damage per second
...
function fireProjectile(tower, target) {
  ...
  const modifiedDamage =
    tower.damage +
    upgrades.damage *
      (tower.type === "strong" ? 10 : tower.type === "sniper" ? 15 : 5);
  ...
  gameState.projectiles.push({
    ...
    poisonDuration:
      tower.type === "poison" ? 4000 + upgrades.special * 1000 : 0,
  });
}
        ...
        if (projectile.type === "splash") {
          gameState.enemies.forEach((nearbyEnemy, nearbyIndex) => {
            ...
            if (nearbyDistance <= projectile.splashRadius) {
              const splashDamage = Math.max(
                projectile.damage *
                  (1 - nearbyDistance / projectile.splashRadius),
          ...
          for (let i = 0; i < 20; i++) {
            gameState.effects.push({
              type: "explosion",
              x:
                enemy.x + (Math.random() - 0.5) * (projectile.splashRadius * 2),
              y:
                enemy.y + (Math.random() - 0.5) * (projectile.splashRadius * 2),
              vx: (Math.random() - 0.5) * 12,
              vy: (Math.random() - 0.5) * 12,
              color: "#f59e0b",
              life: 30,
...
function updateWaveSystem() {
  if (!gameState.showCountdown) {
    ...
    const totalTimeLeft = Math.max(
      0,
      gameState.countdownTimer * 60 + gameState.waveTimer
    );
    ...
    if (gameState.waveTimer <= 0 && gameState.countdownTimer > 0) {
      ...
      document.getElementById("countdown").textContent = Math.max(
        0,
        gameState.countdownTimer
      );
...
function draw() {
  ...
  if (gameState.selectedTower) {
    ...
    drawTowerShape(
      ...
      18,
      towerType.shape,
      isValid ? towerType.color : "#ef4444"
    );
    ctx.globalAlpha = 1;
...
function showTowerUpgrade(towerType) {
  ...
  document.getElementById("damageValue").textContent =
    tower.damage + upgrades.damage;
  document.getElementById("rangeValue").textContent =
    tower.range + upgrades.range * 20;
  document.getElementById("fireRateValue").textContent = Math.max(
    20,
    tower.fireRate - upgrades.fireRate * 10
  );

  document.getElementById("damageLevel").textContent = `Level ${
    upgrades.damage + 1
  }`;
  document.getElementById("rangeLevel").textContent = `Level ${
    upgrades.range + 1
  }`;
  document.getElementById("fireRateLevel").textContent = `Level ${
    upgrades.fireRate + 1
  }`;
  ...
  // Update costs
  document.getElementById("damageBuy").innerHTML = `+${
    towerType === "strong" ? 10 : towerType === "sniper" ? 15 : 5
  } (${50 + upgrades.damage * 25}💰)`;
  document.getElementById("rangeBuy").innerHTML = `+20 (${
    40 + upgrades.range * 20
  }💰)`;
  document.getElementById("fireRateBuy").innerHTML = `+10 (${
    60 + upgrades.fireRate * 30
  }💰)`;
  ...
  if (towerType === "splash") {
    specialLabel.innerHTML = `Exploding Ray <span id="specialLevel">Level ${
      upgrades.special + 1
    }</span>`;
    specialValue.textContent = 50 + upgrades.special * 15;
    specialBuy.innerHTML = `+15 (${80 + upgrades.special * 40}💰)`;
  } else if (towerType === "slow") {
    specialLabel.innerHTML = `Ice Intensity <span id="specialLevel">Level ${
      upgrades.special + 1
    }</span>`;
    specialValue.textContent = `${Math.round(
      (0.5 - upgrades.special * 0.1) * 100
    )}%`;
    specialBuy.innerHTML = `+10% (${80 + upgrades.special * 40}💰)`;
  } else if (towerType === "poison") {
    specialLabel.innerHTML = `Poison Intensity <span id="specialLevel">Level ${
      upgrades.special + 1
    }</span>`;
    specialValue.textContent = `${3 + upgrades.special}/s por ${
      4 + upgrades.special
    }s`;
...
function showTowerConfig(tower, towerIndex) {
  ...
  document.getElementById("targetPriority").value =
    tower.targetPriority || "closest";
...
function updateTowers() {
  gameState.towers.forEach((tower) => {
    ...
    const upgrades = TOWER_UPGRADES[tower.type];
    const modifiedFireRate = Math.max(
      20,
      tower.fireRate - upgrades.fireRate * 10
    );
...
async function fetchData() {
  try {
    const response = await fetch(
      "https://api.github.com/repos/IonMateus/poliguard"
    );
    if (!response.ok) {
    ...
    contentDiv.innerHTML = `
              <div class="flex gap-1">
                <span>⭐</span>
                <span class="text-primary">${data.stargazers_count}</span>
              </div>
              <div class="flex gap-1">
                <span>🍴</span>
                <span class="text-primary">${data.forks_count}</span>
              </div>
    `;
  } catch (error) {
